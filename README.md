...
const MEMORY_KEY = 'nova-agent-memory';
...
function loadMemory() {
  try {
    ...
    Object.assign(agentMemory, parsed);
  } catch (error) {
    console.warn('Unable to load saved memory', error);
  }
}
...
function updateMemoryView() {
  const lines = [
    `Last task: ${agentMemory.lastTask || 'none'}`,
    `Current page: ${agentMemory.currentPage || 'unknown'}`,
    `Plan: ${agentMemory.plan.length ? agentMemory.plan.join(' | ') : 'no plan yet'}`,
    `Confirmations: ${agentMemory.confirmations.length ? agentMemory.confirmations.join(', ') : 'none yet'}`
  ];
  memoryBox.textContent = lines.join('\n');
}
...
function planNextStep() {
  ...
  const plan = [
    ...
    '2. Extract the core product/service structure',
    '3. Summarize the likely user intent and content goal',
    '4. Check pricing or CTA signals',
    '5. Log the result and save memory'
  ];
...
function confirmAction(actionName) {
  ...
  agentMemory.confirmations.push(actionName);
  saveMemory();
  updateMemoryView();
...
function executePlan() {
  const plan = agentMemory.plan.length ? agentMemory.plan : [
    '1. Inspect page',
    '2. Extract data',
    '3. Summarize result'
  ];

  const steps = plan.map((entry, index) => `${index + 1}. ${entry.replace(/^\d+\.\s*/, '')}`);
  const pageInfo = getCurrentPageInfo();
  const actions = [
    toolReadPage(),
    toolExtractData(),
    toolSummarize(),
    `verification()\nFinal page: ${pageInfo.title}\nPath: ${pageInfo.url}`
  ];

  const execution = steps.concat(['', 'Execution trace:']).concat(actions.map((action) => `- ${action.replace(/\n/g, ' | ')}`));

  resultBox.textContent = execution.join('\n');
  addHistory('Execution plan completed');
  updateStatus('Executed', 'success');
}
...
function runLocalAgent(task) {
  ...
  if (/(execute plan|run plan|suorita suunnitelma|ajeta suunnitelma)/.test(lower)) {
    executePlan();
